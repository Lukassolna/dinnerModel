...
function SummaryView(props){
    props.ingredients = sortIngredients(props.ingredients)
    return (
            <div class="debug">
    ...
    */
    function ingredientTableRowCB(ingr){
        ...
        return <tr key={ingr.id} >
                 <td>{ingr.name}</td>
                 <td>{ingr.aisle}</td>
                 <td style="text-align:right;"> {(ingr.amount * props.people).toFixed(2)} {/* multiply by number of people! Display with 2 decimals, use a CSS classs to align right */}</td>
                 <td> {ingr.unit} </td>
               </tr>;
    }
}
