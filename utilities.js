...
function isKnownTypeCB(type) {
  ...
  if (dishTypeRanking[type] !== undefined) {
    return true;
  }
  return false;
...
function dishType(dish) {
  ...
  if (dish.dishTypes.find(isKnownTypeCB) === undefined) {
    return "";
  }
...
function menuPrice(dishesArray) {
  ...
  const totprices = dishesArray.map(callback1);
  ...
  return totprices.reduce(callback2, 0);
}

function callback1(dish) {
